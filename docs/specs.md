...
### 2.7 Release Finalization

Release versioning follows SemVer-oriented classification:

- `MAJOR` for breaking runtime/config/contract changes;
- `MINOR` for important non-breaking code changes or new capabilities;
- `PATCH` for bug fixes and smaller code corrections.

For changelog wording, small code-only cleanups or slight reshaping that do not fix a bug and do not add a feature
should use the standard phrase `Minor refinements`.

Version bumps apply only to code changes. Docs-only edits, comment-only changes, lint-only changes, formatting-only
changes, and similar non-runtime maintenance must not change the release version.

`package.json` is the version source of truth. Release finalization syncs the same version into `src/index.ts` and
`package-lock.json`, and updates `CHANGELOG.md` in the same step.

Each changelog item should stay brief but informative. Use one or two short sentences when needed: first summarize the
problem/change itself, then state the practical user-facing effect or what behavior changed.

When it reads better, put the detail on the next line after the main fix/change summary instead of forcing everything
into one sentence.

Changelog text should read like concise technical prose for humans, not like a rigid semicolon-based template.
...
- a bare gateway domain, such as `gateway-test.com`;
- a URL, including URLs with path and shortener / redirect URLs, such as `https://example.sx/e/abc` or
  `https://shortlink-test.com/xyz`.
...
- quick start on recent `success_since`;
- heuristic fallback;
- replacement logic;
- failure state.
...
For heuristic redirects, collection must be order-independent:

- if a heuristic pattern alias redirects to a pattern final host, both the alias and the final host enter the
  collected working set;
- this does not depend on whether another promise already collected the same final host;
- predicted-mirror cleanup must treat that alias as a real working domain, not as disposable noise.

If the collected working set is mixed and contains both pattern and non-pattern domains, the watcher remains in
pattern mode as long as at least one live pattern domain exists:

- canonical `last_known_mirror` is chosen only from the pattern subset;
- non-pattern tokens from redirect chains do not overwrite `last_known_mirror`;
- non-pattern tokens from `additionalWorkingDomains` do not enter replacement rules for that pattern watcher.
...
**Design rationale**: The starting alias is retained because it is itself a reachable entry point (via redirect),
and losing it from filter rules creates unnecessary churn — the domain appears and disappears between runs. It also
violates the minimal working set contract: any domain that successfully redirects to the final host is a valid
replacement surface and should be kept alongside other working aliases. Alternative (dropping only the starting
alias while keeping neighbors like `+1`, `+2`) was considered surprising and inconsistent.

#### Example: Alias retention with redirects

**Scenario**: `watchers.yml` has `last_known_mirror: testsite65.com` with `force_search_ahead: true`.

**Phase 1** (direct check):
- `testsite65.com` → HTTP 200, redirects to `testsite72.com`
- Result: `hostChanged = true`, `startedHost = testsite65.com`, `newHost = testsite72.com`

**Phase 2** (heuristic search):
- Candidates generated: `testsite66.com`, `testsite67.com`, ..., `testsite71.com`
- All redirect to `testsite72.com` (shared final host)
- `testsite72.com` also found as working domain

**Collected working domains**:
```typescript
foundDomainsPerSite [
  { domain: "testsite65.com", result: { finalHost: "testsite72.com" } },  // starting alias
  { domain: "testsite66.com", result: { finalHost: "testsite72.com" } },  // neighbor
  // ... other neighbors
  { domain: "testsite72.com", result: { finalHost: "testsite72.com" } },  // final host
]
```

**Filter files updated**: All collected domains (`testsite65.com`, `testsite66.com`, ..., `testsite72.com`) appear in
replacement rules. The starting alias `testsite65.com` is **not lost** even though it redirects elsewhere — it
remains a valid entry point that users may have bookmarked or referenced.

This alias-first behavior is intentional. In `many candidates -> one final redirect host` scenarios, the canonical
mirror is the smallest reachable pattern alias, not necessarily the shared final redirect host. The shared final host
is still kept in the replacement surface when it is itself a pattern domain.

The same rule applies to heuristic neighbors. If `testsite66.com` redirects to `testsite72.com` and both are pattern
domains, `testsite66.com` stays in the collected working set even when `testsite72.com` is discovered by a different
candidate earlier or later. Async completion order must not change alias retention.

### failed_since / failed_days / potentially_dead

Auto-updated degradation fields. All date fields use `YYYY-MM-DD HH:MM` format:
...
This branch is entered only when no live pattern domain remains. Seeing a non-pattern redirect target alongside live
pattern mirrors during `force_search_ahead` does not switch the watcher into non-pattern mode.
...
all successful candidates. Example: if `example9.com`, `example10.com`, and `example11.com` are simultaneously
working, canonical should be `example9.com`, not `example10.com`, because natural sort compares the numeric part as a
number,
...
- `success_since`
  Purpose: marks the moment the current continuous successful state began (format: `YYYY-MM-DD HH:MM`).
  Logic: updated **only on actual state transitions**:
  - failure → success (exit from `failed_since`);
  - domain change (`hostChanged = true`, including successful heuristic replacement);
  - pattern → non-pattern phase transition (`historyUpdated && !shouldUpdate`);
  - antibot-accepted success where the effective host actually changed.
  Repeated success on the same `last_known_mirror` without any state change does NOT rewrite `success_since`,
  to avoid unnecessary diff noise in `watchers.yml`. Used by the "recent success" optimization when the script
  first tries `last_known_mirror`.
  **Removed**: On success → failure transition (real failure or pattern-change failure), `success_since` is
  deleted to keep failed state clean — only `failed_since`, `failed_days`, `potentially_dead` remain.
  When the site recovers, `success_since` is set again in success branches.
- Legacy: `last_seen` is accepted at load time and migrated to `success_since`; never persisted.

- `failed_since`
  Purpose: stores the moment the current continuous failure series began (format: `YYYY-MM-DD HH:MM`).
...
Fields `last_known_mirror`, `success_since`, `failed_since`, `failed_days`, `potentially_dead`, `pattern_changed`,
...
- If `success_since` is recent, the code first tries `last_known_mirror`, but on failure must fall back to `initial_domain`
  if it is set.
...
```text
if site.success_since is recent (< 2 days) and site.last_known_mirror exists:
...
### 5.5a Early Exit on JS Redirect When probe_text Matched

When `probe_text` is configured and all probe strings are found on the current (intermediate) domain, and the next
redirect in the chain is a client-side JS redirect (`location.replace`, `window.location`, `location.href`), the
resolver stops following the redirect chain and returns the current domain as successful.

**Rationale:** Sites use JS redirects (`location.replace()`) as anti-cloning protection or to redirect to decoy pages.
The real working site contains `probe_text`, but a JS redirect points to a decoy. Without early exit, the resolver
would follow the JS redirect to the decoy, where `probe_text` is absent, and mark the site as failed.

**Behavior:**

- If `probe_text` is matched on the current domain AND the next redirect is a JS redirect → return success with the
  current domain as `finalHost`, with `probeTextMatchedBeforeJsRedirect: true`.
- If `probe_text` is matched but the next redirect is a meta refresh → continue following (meta refresh is server-like
  and typically legitimate).
- If `probe_text` is NOT matched → continue following (old behavior).
- If `probe_text` is NOT configured → continue following (old behavior).

**Integration with `force_search_ahead`:**

- When `force_search_ahead` is enabled and early exit occurs, `shouldTriggerHeuristic` is set to `true`.
- This allows the heuristic to continue searching for additional working domains even after the primary domain is
  found via early exit.
- The heuristic generates candidates from the domain where `probe_text` matched (the `finalUrl` in the early exit
  result), not from the decoy domain.

**Meta refresh is NOT affected:** Meta refresh (`<meta http-equiv="refresh">`) is treated as a server-like redirect
and is always followed, regardless of `probe_text` match status.
...
For pattern watchers, this selection is applied to the pattern subset of the working set. If a mixed set contains
`example2069.com`, `example2073.com`, and `gateway-test.com`, the canonical mirror is chosen from
`[example2069.com, example2073.com]`; `gateway-test.com` is ignored for canonical state and replacements.

If many working aliases redirect to one shared final host, the canonical mirror is still the smallest reachable
pattern alias. The final redirect host may remain in `additionalWorkingDomains` and in filter replacements, but it
does not override the alias-based canonical selection unless it is itself the smallest pattern domain.
...
- `exampletv18.live < exampletv20.live`
...
Single-domain wrapper forms such as `[$domain=example123.com]` are processed by the same domain-list pipeline as
ordinary single-domain `$domain=example123.com`: replacement, predicted-mirror cleanup, working-set expansion, and
final deduplication must behave identically.
...
```yaml
initial_domain: gateway-test.com
```
...
```text
gateway-test.com -> mirror86-test.com
```

because `gateway-test.com` might be a stable entry point that should remain in filters.
...
- `gateway-test.com` is used only to find the current mirror;
- `gateway-test.com` is not replaced in filter files;
...
initial_domain: gateway-test.com
replace_initial_domain: false
last_known_mirror: mirror85-test.com
```
...
```yaml
last_known_mirror: mirror86-test.com
```

Replacement:

```text
mirror85-test.com -> mirror86-test.com
```
...
```text
if first failure:
    failed_since = now              # YYYY-MM-DD HH:MM
    failed_days = 0
    potentially_dead = true
...
# State transition: success → failed
delete success_since              # Remove to keep failed state clean
```
...
- `success_since` is updated (format: `YYYY-MM-DD HH:MM`) **only on actual state transitions**:
  - domain change (`hostChanged = true`);
  - exit from prior failure (`failed_since` was set before this run);
  - pattern → non-pattern phase transition (`historyUpdated && !shouldUpdate`);
  - antibot-accepted success where effective host actually changed.
  On repeated identical success without state change, `success_since` is preserved (noise suppression);
...
- repeated success on the same `last_known_mirror` should not update `success_since` without actual state transition;
...
Used when `gateway-test.com` is needed only as stable entry point and replacements in filter files should execute only
...
sites:
  Example:
    initial_domain: https://mirror-test.com/e/xyz
    path: e/xyz
```
...
Important: `No changes detected` and `Unchanged watchers` list should be determined by comparing final `newHost` with
...
`Unchanged watchers` should identify the watcher first and show the active mirror only as context, for example
`WatcherName (active.example123.com)`, instead of listing bare domains with no watcher name.
...
### 17.1 Release Versioning Rules

- Version and changelog updates happen only when a task/workstream is being finalized.
- The finalization step must update both the current session handoff and `CHANGELOG.md`.
- Use the SemVer classification from section 2.7 to choose `patch`, `minor`, or `major`.
- Changelog entries must describe code changes only; exclude docs/comments/lint-only maintenance.
